...
class TaskRepository(private val tasksDao: TaskDao) {
    ...
    fun getTasks(filter: TasksFilterType): LiveData<PagedList<Task>> {
        ...
        val filterData = FilterUtils.getFilteredQuery(filter)

        val setFilterData = tasksDao.getTasks(filterData)

        val dataConfig = PagedList.Config.Builder().setPageSize(PAGE_SIZE).setEnablePlaceholders(PLACEHOLDERS).build()

//        throw NotImplementedError("Not yet implemented")
        return  LivePagedListBuilder(setFilterData, dataConfig).build()
    }
